...
`+JSON.stringify(o,null,2):u=" "+this.formatData(o));let m="";if(n){let{sessionId:E,memorySessionId:f,correlationId:h,...l}=n;Object.keys(l).length>0&&(m=` {${Object.entries(l).map(([b,O])=>`${b}=${O}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${_}${s}${m}${u}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=_?`${_[1].split("/").pop()}:${_[2]}`:"unknown",m={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,m,n),o}},p=new V;var Ct={};function Ot(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,ce.fileURLToPath)(Ct.url))}var Rt=Ot();function Nt(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,S.join)((0,K.homedir)(),".claude-mem"),e=(0,S.join)(r,"settings.json");try{if((0,k.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var N=Nt(),I=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,K.homedir)(),".claude"),Gt=(0,S.join)(I,"plugins","marketplaces","thedotmack"),Ht=(0,S.join)(N,"archives"),Bt=(0,S.join)(N,"logs"),Wt=(0,S.join)(N,"trash"),Yt=(0,S.join)(N,"backups"),qt=(0,S.join)(N,"modes"),Vt=(0,S.join)(N,"settings.json"),pe=(0,S.join)(N,"claude-mem.db"),Kt=(0,S.join)(N,"vector-db"),Jt=(0,S.join)(N,"observer-sessions"),Qt=(0,S.join)(I,"settings.json"),zt=(0,S.join)(I,"commands"),Zt=(0,S.join)(I,"CLAUDE.md");function _e(r){(0,k.mkdirSync)(r,{recursive:!0})}function ue(){return(0,S.join)(Rt,"..")}var me=require("crypto");var At=3e4;function w(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function $(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var F=class{db;constructor(e=pe){e!==":memory:"&&_e(N),this.db=new le.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.ensureReadCountColumns()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),_=w(e,s.title,s.narrative),u=$(this.db,_,a);if(u)return{id:u.id,createdAtEpoch:u.created_at_epoch};let T=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),_=new Date(d).toISOString();return this.db.transaction(()=>{let m=[],T=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let h=w(e,f.title,f.narrative),l=$(this.db,h,d);if(l){m.push(l.id);continue}let g=T.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),o||null,i,h,_,d);m.push(Number(g.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,_,d);E=Number(h.lastInsertRowid)}return{observationIds:m,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,_){let u=_??Date.now(),m=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],f=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let b=w(e,g.title,g.narrative),O=$(this.db,b,u);if(O){E.push(O.id);continue}let lt=f.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),a||null,d,b,m,u);E.push(Number(lt.lastInsertRowid))}let h;if(n){let b=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,m,u);h=Number(b.lastInsertRowid)}return this.db.prepare(`
    ...
    `).all(..._)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,_;if(e!==null){let l=`
      ...
      `,g=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let b=this.db.prepare(l).all(t,...a,s),O=this.db.prepare(g).all(t,...a,n+1);if(b.length===0&&O.length===0)return{observations:[],sessions:[],prompts:[]};d=b.length>0?b[b.length-1].created_at_epoch:t,_=O.length>0?O[O.length-1].created_at_epoch:t}catch(b){return p.error("DB","Error getting boundary timestamps",void 0,{error:b,project:o}),{observations:[],sessions:[],prompts:[]}}}let u=`
      SELECT *
      FROM observations
...
`):null}function Dt(r,e,t,s,n,o){let i=[];o?i.push(...Ye(r)):i.push(...De(r));let a=null,d="",_=!1;for(let u of e)if(u.type==="summary"){_&&(i.push(""),_=!1,a=null,d="");let m=u.data,T=st(m.displayTime);o?i.push(...Je(m,T)):i.push(...we(m,T))}else{let m=u.data,T=ot(m.files_modified,n,m.files_read),E=rt(m.created_at),f=E!==d,h=f?E:"";d=E;let l=t.has(m.id);if(T!==a&&(_&&i.push(""),o?i.push(...qe(T)):i.push(...xe(T)),a=T,_=!0),l){let g=Mt(m,s);o?i.push(...Ke(m,E,f,g,s)):(_&&!o&&(i.push(""),_=!1),i.push(...ke(m,h,g,s)),a=null)}else o?i.push(Ve(m,E,f,s)):i.push(Ue(m,h,s))}return _&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=Lt(r);for(let[a,d]of i)o.push(...Dt(a,d,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,c.blue)),t.push(...x("Learned",r.learned,c.yellow)),t.push(...x("Completed",r.completed,c.green)),t.push(...x("Next Steps",r.next_steps,c.magenta))):(t.push(...D("Investigated",r.investigated)),t.push(...D("Learned",r.learned)),t.push(...D("Completed",r.completed)),t.push(...D("Next Steps",r.next_steps))),t}function ct(r,e){return e?Qe(r):$e(r)}function pt(r,e,t){return!G(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?ze(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var xt=_t.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Ut(){try{return new F}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,mt.unlinkSync)(xt)}catch(e){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function kt(r,e){return e?Ze(r):Pe(r)}function wt(r,e,t,s,n,o,i){let a=[],d=Z(e);a.push(...et(r,d,s,i));let _=t.slice(0,s.sessionCount),u=Ne(_,t),m=re(e,u),T=Ce(e,s.fullObservationCount);a.push(...it(m,T,s,n,i));let E=t[0],f=e[0];at(s,E,f)&&a.push(...dt(E,i));let h=se(e,s,o,n);return a.push(...ct(h,i)),a.push(...pt(d,s,i)),a.join(`
`).trimEnd()}async function oe(r,e=!1){let t=J(),s=r?.cwd??process.cwd(),n=Te(s),o=r?.projects||[n],i=Ut();if(!i)return"";try{let a=o.length>1?Oe(i,o,t):ee(i,n,t),d=o.length>1?Re(i,o,t):te(i,n,t);return a.length===0&&d.length===0?kt(n,e):wt(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
