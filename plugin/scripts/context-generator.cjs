...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",l={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},m=new W;var Ct={};function bt(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Ct.url))}var Ot=bt(),I=y.get("CLAUDE_MEM_DATA_DIR"),Y=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Ht=(0,h.join)(I,"archives"),Wt=(0,h.join)(I,"logs"),Yt=(0,h.join)(I,"trash"),Vt=(0,h.join)(I,"backups"),qt=(0,h.join)(I,"modes"),Kt=(0,h.join)(I,"settings.json"),me=(0,h.join)(I,"claude-mem.db"),Jt=(0,h.join)(I,"vector-db"),zt=(0,h.join)(I,"observer-sessions"),Qt=(0,h.join)(Y,"settings.json"),Zt=(0,h.join)(Y,"commands"),es=(0,h.join)(Y,"CLAUDE.md");function ue(r){(0,pe.mkdirSync)(r,{recursive:!0})}function le(){return(0,h.join)(Ot,"..")}var $=class{db;constructor(e=me){e!==":memory:"&&ue(I),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).get(e).count}createSDKSession(e,t,s){let n=new Date,o=n.getTime();this.db.prepare(`
      INSERT OR IGNORE INTO sdk_sessions
      (content_session_id, memory_session_id, project, user_prompt, started_at, started_at_epoch, status)
      VALUES (?, NULL, ?, ?, ?, ?, 'active')
    `).run(e,t,s,n.toISOString(),o);let i=this.db.prepare("SELECT id, project FROM sdk_sessions WHERE content_session_id = ?").get(e);return t&&t!==i.project&&(this.db.prepare("UPDATE sdk_sessions SET project = ? WHERE id = ?").run(t,i.id),m.info("SESSION","Updated project for existing session",{sessionId:i.id,oldProject:i.project||"(empty)",newProject:t})),i.id}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime();return this.db.prepare(`
  ...
  `).all(...e,...s,...o,t.totalObservationCount)}function Ce(r,e,t){let s=e.map(()=>"?").join(",");return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(...e,t.sessionCount+q)}function Rt(r){return r.replace(/\//g,"-")}function Nt(r){try{if(!(0,F.existsSync)(r))return{userMessage:"",assistantMessage:""};let e=(0,F.readFileSync)(r,"utf-8").trim();if(!e)return{userMessage:"",assistantMessage:""};let t=e.split(`
